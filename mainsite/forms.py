...
class MemberForm(forms.ModelForm):
    class Meta:
        ...
        help_texts = {
            'branch': _('Your current branch'), 'work':_('Keep empty if you are not employed anywhere.'), 'year':_('Your current Year, select Alumni if passout'),
...
'''
        widget = {
            ...
            'DOB': forms.DateInput(attrs={'class':'inputfield w3-input w3-border', 'type':'date',}),
            ...
            'active': forms.TextInput(attrs={'class':'inputfield w3-check w3-border',}),
        }
'''
...
class ProjectForm(forms.ModelForm):
    class Meta:
        ...
        help_texts = {
            'pic':_('If present'), 'github':_('If present'), 'completed':_('Tick if project is complete'), 'contributers':_('Ctrl + Click to choose'),
        }
'''
        widget = {
